...
export const deleteExpense = async (id: string): Promise<void> => {
	console.log(`Attempting to send delete request for ID: ${id}`);
	...
	if (!response.ok) {
		throw new Error(`Failed to delete expense with status: ${response.status}`);
	}
	console.log("Expense deleted successfully on the backend");
};
